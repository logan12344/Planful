...
class SelectEventTypeDialog(
    ...
    val currEventType: Long,
    val showCalDAVCalendars: Boolean,
    val showNewEventTypeOption: Boolean,
    ...
    init {
        val view =
            activity.layoutInflater.inflate(R.layout.dialog_select_radio_group, null) as ViewGroup
        ...
        activity.eventsHelper.getEventTypes(activity, showOnlyWritable) {
            ...
            activity.runOnUiThread {
                if (addLastUsedOneAsFirstOption) {
                    val lastUsedEventType = EventType(
    ...
    private fun addRadioButton(eventType: EventType) {
        ...
        if (eventType.color != Color.TRANSPARENT) {
            view.dialog_radio_color.setFillWithStroke(
                eventType.color,
